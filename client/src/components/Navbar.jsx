...
const Navbar = () => {
  ...
  return (
    <div className="relative z-40 w-full border-b border-black/5 bg-[rgba(252,249,245,0.96)] backdrop-blur-xl">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 md:py-4 lg:px-8">
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          {!user ? (
            <Link
              to="/login"
              className="rounded-full bg-[#B21A15] px-3.5 py-2 text-sm font-semibold text-white"
            >
              Login
            </Link>
          ) : (
      ...
      >
        <div
          className={`absolute inset-0 bg-[rgba(15,23,42,0.18)] backdrop-blur-xl transition-opacity ${
        ...
        <aside
          ...
          className={`fixed left-0 top-0 flex h-[100dvh] w-[min(88vw,360px)] flex-col overflow-y-auto border-r border-white/40 bg-[linear-gradient(180deg,rgba(252,248,244,0.62),rgba(248,240,234,0.54))] px-5 pb-6 pt-5 shadow-[0_24px_60px_rgba(15,23,42,0.24)] backdrop-blur-[28px] transition-transform ${
        ...
        >
          <div className="pointer-events-none absolute inset-0">
            ...
            <div className="absolute right-0 top-0 h-52 w-52 rounded-full bg-white/45 blur-3xl" />
            <div className="absolute bottom-24 left-8 right-8 h-px bg-gradient-to-r from-transparent via-[#B21A15]/25 to-transparent" />
          </div>

          <div className="relative flex items-center justify-between gap-3 border-b border-black/5 pb-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#B21A15]">
                PrintedTeez
              </p>
          ...
          <div className="relative mt-5 shrink-0">
            <div className="flex items-center rounded-full border border-white/55 bg-white/62 px-4 py-3 shadow-[0_12px_30px_rgba(15,23,42,0.08)] backdrop-blur-md">
              <Search size={16} className="text-gray-400" />
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
            ...
            {suggestions.length > 0 && (
              <div className="mt-3 max-h-72 overflow-y-auto rounded-[1.2rem] border border-white/50 bg-white/74 shadow-lg backdrop-blur-md">
                {suggestions.slice(0, 5).map((item) => (
                  <button
                    key={item._id}
                    onClick={() => handleSelectProduct(item._id)}
                ...
                className={({ isActive }) =>
                  `rounded-2xl px-4 py-3 text-base font-medium backdrop-blur-sm transition ${
                    isActive
                      ...
                      : "border border-white/18 bg-white/22 text-gray-800 hover:border-white/35 hover:bg-white/34"
                  }`
                }
              >
                {item.label}
          ...
          <div className="relative mt-6 grid shrink-0 grid-cols-2 gap-3 border-t border-black/5 pt-5">
            <Link
              to="/cart"
              onClick={() => setMobileMenuOpen(false)}
              className="rounded-2xl border border-white/55 bg-white/62 px-4 py-3 text-center text-sm font-medium text-gray-800 shadow-[0_10px_24px_rgba(15,23,42,0.08)] backdrop-blur-md"
            >
              Cart
            </Link>
            <Link
