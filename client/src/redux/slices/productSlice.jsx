...
export const createProduct = (formData) => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.post("/product/admin/create-product", formData, {
...
export const updateProduct = (id, formData) => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.put(`/product/admin/update-product/${id}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    dispatch(productStart());
    await api.delete(`/product/admin/delete-product/${id}`);
...
export const toggleFeatured = (id) => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.patch(`/product/admin/toggle-featured/${id}`);
...
export const suggestSKU = (title) => async () => {
  try {
    const { data } = await api.get(`/product/admin/sku-suggest?title=${encodeURIComponent(title)}`);
...
export const fetchAdminCategories = () => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.get("/product/admin/categories");
    dispatch(adminCategoryListSuccess(data));
  } catch (err) {
    dispatch(
      productFail(err.response?.data?.message || "Failed to load admin categories")
    );
  }
};
...
export const createCategory = (formData) => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.post("/product/admin/categories", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    dispatch(categoryCreated(data.category));
    return { ok: true, category: data.category };
  } catch (err) {
    dispatch(
      productFail(err.response?.data?.message || "Failed to create category")
    );
    return {
      ok: false,
...
export const updateCategory = (id, formData) => async (dispatch) => {
  try {
    dispatch(productStart());
    const { data } = await api.put(`/product/admin/categories/${id}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    ...
    dispatch(
      productFail(err.response?.data?.message || "Failed to update category")
    );
    return {
      ok: false,
...
export const deleteCategory = (id) => async (dispatch) => {
  try {
    dispatch(productStart());
    await api.delete(`/product/admin/categories/${id}`);
    dispatch(categoryDeleted(id));
    return { ok: true };
  } catch (err) {
    dispatch(
      productFail(err.response?.data?.message || "Failed to delete category")
    );
    return {
      ok: false,
...
const products = createSlice({
  ...
  reducers: {
    ...
    adminCategoryListSuccess: (state, action) => {
      state.loading = false;
    ...
    categoryCreated: (state, action) => {
      state.loading = false;
      state.adminCategories = [...state.adminCategories, action.payload].sort(
    ...
    categoryUpdated: (state, action) => {
      state.loading = false;
      const updated = action.payload;
      state.adminCategories = state.adminCategories
